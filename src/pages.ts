...
export async function setupPages({ localeCodes, options, normalizedLocales }: I18nNuxtContext, nuxt: Nuxt) {
  const gen = {
    i18nPathToPath: {} as Record<string, string>,
    pathToI18nConfig: {} as Record<string, Record<string, string | boolean>>,
    pathLocalePaths: new Map<string, Set<string>>(),
    regexPaths: [] as string[],
    temp: [] as any[],
  }
...
export const pathToI18nConfig = ${JSON.stringify(gen.pathToI18nConfig, null, 2)};
export const i18nPathToPath = ${JSON.stringify(gen.i18nPathToPath, null, 2)};
export const regexPaths = ${JSON.stringify(gen.regexPaths, null, 2)};
export const temp = ${JSON.stringify(gen.temp, null, 2)};
`
// export const pathLocalePaths = ${JSON.stringify([...gen.pathLocalePaths.entries()].map(([key, value]) => [key, [...value]]), null, 2)};
    },
  })
  ...
  nuxt.hook(
    ...
    async (pages) => {
      ...
      gen.i18nPathToPath = {}
      gen.pathToI18nConfig = {}
      gen.pathLocalePaths = new Map<string, Set<string>>()
      gen.regexPaths = []
      for (const [path, localeConfig] of Object.entries(ctx.pathToConfig)) {
        const resPath = resolveRoutePath(path)
        gen.pathToI18nConfig[resPath] ??= {}
        // if (!gen.pathLocalePaths.has(resPath)) {
        //   gen.pathLocalePaths.set(resPath, new Set())
        // }
        for (const [locale, localePath] of Object.entries(localeConfig!)) {
          const localized = localePath === true ? path : localePath
          gen.pathToI18nConfig[resPath][locale] = localized && resolveRoutePath(localized)
          if (gen.pathToI18nConfig[resPath][locale]) {
            if (!gen.pathLocalePaths.has(gen.pathToI18nConfig[resPath][locale])) {
              gen.pathLocalePaths.set(gen.pathToI18nConfig[resPath][locale], new Set())
            }
            gen.pathLocalePaths.get(gen.pathToI18nConfig[resPath][locale])!.add(locale)
            gen.i18nPathToPath[gen.pathToI18nConfig[resPath][locale]] = resPath
          } else {
            gen.pathLocalePaths.set(resPath, new Set())
          }
        }
      ...
      // const t: any[] = []
      gen.temp = []
      for (const [key, value] of gen.pathLocalePaths) {
        const fragments = []
        if (value.size > 0) {
          fragments.push(`/:locale(${Array.from(value).join('|')})?`)
        }
        fragments.push(key === '/' ? '' : key)
        gen.regexPaths.push(withoutTrailingSlash(fragments.join('')))

        // const foreignPaths = Array.from(value).map((locale) => {
        //   console.log(gen.i18nPathToPath[key], key)
        //   return gen.pathToI18nConfig?.[gen.i18nPathToPath[key]]
        // }).filter(Boolean)

        // gen.temp.push({
        //   path: key,
        //   foreignPaths,
        //   // regexPath: withoutTrailingSlash(fragments.join('')),
